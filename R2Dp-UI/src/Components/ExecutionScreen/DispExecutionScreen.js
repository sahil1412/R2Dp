...
import execDISPPOApiCollection from "../../apiservices/ExecutePO/executeDISPPO.api";
import { ToastContainer, toast,Slide } from 'react-toastify';
import dispApiCollection from '../../apiservices/ERP/DISP.api';
const DispExecutionScreen = () => {
  ...
  useEffect(() => {
   fetchDISPData()
   fetchPoType()
  },[fetchDISPData,fetchPoType,dispData,forcePONumber]);
  const handleExecution=(PONumber)=>
  {
  execDISPPOApiCollection.executePO(PONumber)
   .then((data)=>
     {
  ...
  const forceStopPO=(e)=>
  {
   ...
   dispApiCollection.changeStatus(forcePONumber,status)
    .then((data)=>
     {
       if(data.data.success)
       {
  ...
  return (
    ...
    <h3>Dispatch Execution Screen</h3>
    </div> 
    <div className='row'>
    ...
    <TableContainer className='table-container' component={Paper}>
      <Table aria-label="customized table">
        ...
        <TableBody>
         {dispData.filter((item)=>item.Status!=="Open").map((row) => {
...
export default DispExecutionScreen